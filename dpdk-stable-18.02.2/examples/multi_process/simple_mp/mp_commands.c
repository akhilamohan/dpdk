		...
		__attribute__((unused)) void *data)
{
	...
	aes_encrypt((unsigned char *)msg, 4, aes_key, sizeof(aes_key), &enc_out , &enc_len);
	enc_out[15] = '\0';
	print_data("\n Encrypted", enc_out, enc_len);
	if (rte_ring_enqueue(send_ring, (char *)enc_out) < 0) {
		printf("Failed to send message - message discarded\n");
		rte_mempool_put(message_pool, (char *)enc_out);
	}
}
